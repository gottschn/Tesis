...
const CiudadList: React.FC<{ ciudad: CiudadesProps }> = ({ ...props }) => {
  ...
  const { ciudades, filter } = HelperRedux.useSelector(
    (state) => state.ciudades
  );
  const [currentCiudad, setCurrentCiudad] = useState<CiudadesProps>(
    {} as CiudadesProps
  );
  ...
  const handleDeleteCiudades = () => {
    ...
    Swal.fire({
      icon: "success",
      text: "La Ciudad se elimino con exito.",
    ...
    <>
      ...
      <DataGrid
        ...
        filterComponent={(onClosedFilter) => (
          <CiudadFilter onClosed={onClosedFilter} />
        )}
      />
      <ModalConfirmation
