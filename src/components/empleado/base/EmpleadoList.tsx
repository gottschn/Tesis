...
const EmpleadoList: React.FC<{ empleados: EmpleadosProps }> = ({
  ...props
}) => {
  const dispatch = HelperRedux.useDispatch();
  const { empleados, filter } = HelperRedux.useSelector(
    (state) => state.empleados
  );
  const [currentEmpleado, setCurrentEmpleado] = useState<EmpleadosProps>(
    {} as EmpleadosProps
  );
  ...
  useEffect(() => {
    if (empleados.length === 0) getInitial();
  }, []);
    ...
    <>
      ...
      <DataGrid
        ...
        rows={empleados
          .filter(
            (x) =>
              x.apynom.includes(filter.apynom) &&
              x.areaTrabajo.includes(filter.areaTrabajo)
          )
          .map((x) => ({
            ...x,
        ...
        filterComponent={(onClosedFilter) => (
          <EmpleadoFilter onClosed={onClosedFilter} />
        )}
      />
