...
const PagoList: React.FC<{ pago: PagosProps }> = ({ ...props }) => {
  ...
  const [currentPagos, setCurrentPagos] = useState<PagosProps>(
    {} as PagosProps
  );

  useEffect(() => {
    if (pagos.length === 0) getInitial();
  }, []);
  ...
  const handleDeletePagos = () => {
    ...
    Swal.fire({
      icon: "success",
      text: "El Pago se elimino con exito.",
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        rows={pagos
          .filter((x) => x.legajo.includes(filter.legajo))
          .map((x) => ({
            ...x,
        ...
        filterComponent={(onClosedFilter) => (
          <PagoCuotasFilter onClosed={onClosedFilter} />
        )}
      />
