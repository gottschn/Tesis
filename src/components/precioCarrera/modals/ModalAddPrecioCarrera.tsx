...
const ModalAddPrecioCarrera = () => {
  ...
  const handleSubmit = (e: any) => {
    ...
    createPrecioCarreras(form.monto, form.matricula, form.fecha, form.carreraId)
      .then((x) => {
        dispatch(
          Actions.createPrecioCarreras({
            ...form,
            id: x.data.value,
          })
        );
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onFocus={() => setErrorMsg(null)}
              >
                <option key={`option-carrera-0`} value={0}>
                  Seleccione...
                </option>
                {carreras.carreras.map((x) => (
                  <option
                    key={`option-carrera-${x.id}`}
                    value={x.id}
                  >{`Codigo:${x.id} - Carrera:${x.descripcion}`}</option>
                ))}
              </Form.Control>
            </Form.Group>
