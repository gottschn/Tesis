...
const UsuarioList: React.FC<{ usuario: UsuarioProps }> = ({ ...props }) => {
  ...
  const { usuarios, filter } = HelperRedux.useSelector(
    (state) => state.usuarios
  );
  const [currentUsuario, setCurrentUsuario] = useState<UsuarioProps>(
    {} as UsuarioProps
  );
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        rows={usuarios
          .filter((x) => x.nombre.includes(filter.nombre))
          .map((x) => ({
            ...x,
          }))}
        filterComponent={(onClosedFilter) => (
          <UsuarioFilter onClosed={onClosedFilter} />
        )}
      />
